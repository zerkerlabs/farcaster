...
// master key from the ZERKER_KMS_KEY environment variable (hex-encoded,
// 64 characters). If ZERKER_KMS_KEY is not set, a random ephemeral key is
...
// production environment without setting ZERKER_KMS_KEY.
type LocalProvider struct {
	masterKey []byte // 32 bytes, AES-256
}

// NewLocalProvider returns a LocalProvider. The master key is read from
// ZERKER_KMS_KEY (64 hex chars = 32 bytes). When the variable is absent a
// random ephemeral key is generated and a warning is implied by the name.
func NewLocalProvider() (*LocalProvider, error) {
	if raw := os.Getenv("ZERKER_KMS_KEY"); raw != "" {
		key, err := hex.DecodeString(raw)
		if err != nil {
			return nil, fmt.Errorf("kms: decode ZERKER_KMS_KEY: %w", err)
		}
		if len(key) != 32 {
			return nil, fmt.Errorf("kms: ZERKER_KMS_KEY must be 32 bytes (64 hex chars), got %d bytes", len(key))
		}
