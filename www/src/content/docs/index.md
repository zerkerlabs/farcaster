---
title: What is Zerker Gateway
description: Zerker Gateway is a sovereign, single-binary gateway that routes, observes, and meters agent traffic — self-hosted, with no server-held key.
---

Zerker Gateway is the gateway to **manage, analyze, and productize agents and
...
- **A catalog** — register an agent or tool once (a name, an upstream URL,
  optional credentials), and callers reach it through the gateway instead of
  directly.
...
- **A payment gate** — price a route or a specific MCP tool, and the gateway will
