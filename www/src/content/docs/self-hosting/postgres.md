...
```bash
ZERKER_DATABASE_URL=postgres://user:pass@db:5432/zerker?sslmode=require
```
...
is advanced as needed. See [Upgrades](/self-hosting/upgrades/) for what
this means during a rolling deploy.

## The in-memory fallback (dev only)

If neither `ZERKER_DATABASE_URL` nor `DATABASE_URL` is set, the gateway
...
- Back up the database on your normal cadence: it holds the (envelope-encrypted)
  ...
  [KMS key](/self-hosting/kms-and-secrets/).
