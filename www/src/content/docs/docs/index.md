...
description: Farcaster is a sovereign, single-binary gateway that routes, observes, and meters agent traffic — self-hosted, with no server-held key.
---
...
- **A catalog** — register an agent or tool once (a name, an upstream URL,
  optional credentials), and callers reach it through Farcaster instead of
  directly.
...
- **A payment gate** — price a route or a specific MCP tool, and Farcaster will
...
- **The self-hoster** — sovereign, regulated, or air-gapped operators who need
  ...
  else's cloud. See [Sovereignty & no-custody](/docs/concepts/sovereignty/).
...
- [Install](/docs/install/) — run the gateway locally in one command.
- [Quickstart](/docs/quickstart/) — register an agent, proxy a call, gate a
  paid tool.
- [Concepts](/docs/concepts/architecture/) — how the gateway, the facilitator,
