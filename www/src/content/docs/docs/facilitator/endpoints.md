...
2. **Independently re-verify** — signature recovery, `value` within
   ...
   (see [Custody posture](/docs/facilitator/custody/)).
...
5. **Submit and wait for first-block inclusion** — signs and broadcasts
   `transferWithAuthorization` via the configured [signer](/docs/facilitator/signers/),
...
`settlement_failed` (see [Gate vs settle](/docs/payments/gate-vs-settle/)).
...
[The wire contract](/docs/payments/wire-contract/), generated from
