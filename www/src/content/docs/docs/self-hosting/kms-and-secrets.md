---
title: KMS & secrets
description: How the gateway envelope-encrypts stored credentials, the FARCASTER_KMS_KEY master key, and secret-handling hygiene.
---
...
## The master key: `FARCASTER_KMS_KEY`

The built-in KMS provider takes its master key from `FARCASTER_KMS_KEY`, a
...
If `FARCASTER_KMS_KEY` is **not** set, the gateway generates a **random
...
tests. **Never run production without setting `FARCASTER_KMS_KEY`.**
...
**Rotating `FARCASTER_KMS_KEY` itself is not supported today.** The built-in
...
new master key at once. If you change `FARCASTER_KMS_KEY` and restart, every
...
- Deliver `FARCASTER_KMS_KEY`, `FARCASTER_DATABASE_URL`, and any provider
