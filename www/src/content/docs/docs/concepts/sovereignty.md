...
description: Why Farcaster never holds a private key, and why that's the point for self-hosters.
---

Farcaster's positioning is **sovereign, single-binary, no-custody**. Each word
...
- **No-custody.** This is the sharpest edge. When a caller pays for a gated
  tool call, Farcaster verifies the payment authorization's signature and
  ...
  on your behalf: Farcaster's gateway simply never has a key to lose, leak, or
...
also self-host. See [Facilitator](/docs/facilitator/) and
[The open-core boundary](/docs/concepts/open-core-boundary/) for where that
capability sits.
