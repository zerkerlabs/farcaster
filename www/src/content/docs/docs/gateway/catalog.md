...
- **`pending`** — no `upstream_url` set. The catalog entry exists but is not
  invocable; see [Routing & proxy](/docs/gateway/proxy/#pending-agents-are-not-invocable).
...
[MCP-native transport](/docs/gateway/mcp/). `GET /v1/agents?protocol=mcp`
...
[Auth & multi-tenancy](/docs/concepts/auth-and-multi-tenancy/). Registering a
