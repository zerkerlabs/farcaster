...
Once an agent has an `upstream_url` (see [Agent Catalog](/docs/gateway/catalog/)),
...
every call is addressable in [observability & analytics](/docs/gateway/observability/)
...
- **`X-Farcaster-Model`** — optional, caller-supplied, recorded on the
  ...
  [Observability & analytics](/docs/gateway/observability/)).
...
[Agent Catalog](/docs/gateway/catalog/#credentials-are-a-separate-resource)).
...
invocation record is created. See [Payments](/docs/payments/) and
[Quickstart → Gate a paid tool](/docs/quickstart/#3-gate-a-paid-tool).
