...
The [routing & proxy](/docs/gateway/proxy/) layer is protocol-agnostic by
...
MCP servers; see [Agent Catalog](/docs/gateway/catalog/).
...
agent (see [Routing & proxy](/docs/gateway/proxy/)) — the request body is a
...
`search_docs`." See [Observability & analytics](/docs/gateway/observability/).

## Retry safety

The proxy retries transient upstream failures (see
[Routing & proxy](/docs/gateway/proxy/#credentials-retries-and-failure-modes)).
