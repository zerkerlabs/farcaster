...
Every call the [proxy](/docs/gateway/proxy/) forwards is already recorded in
...
[MCP method/tool](/docs/gateway/mcp/#method-and-tool-level-observability) and
...
[agent-level toggle](/docs/gateway/catalog/), off by default) **and** the
...
- **`model`** — caller-supplied only, via the `X-Farcaster-Model` request
  header (see [Routing & proxy](/docs/gateway/proxy/#body-verbatim-metadata-in-headers)).
  ...
  [MCP-native transport](/docs/gateway/mcp/).
...
as every other surface (see [Auth & multi-tenancy](/docs/concepts/auth-and-multi-tenancy/)).
