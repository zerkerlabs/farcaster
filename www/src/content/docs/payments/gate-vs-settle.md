...
1. A request to a priced route with no `X-PAYMENT` header gets `402` with a
   challenge (see [The wire contract](/payments/wire-contract/)).
...
— see [Facilitator](/facilitator/)) turns the same priced route into a
...
facilitator. See [Custody posture](/facilitator/custody/) for exactly
who holds what.
